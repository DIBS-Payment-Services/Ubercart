...
class dibsflex_api extends dibsflex_helpers {
    ...
    function dibsflex_api_getInvoiceXML($oOrderInfo) {
        ...
        foreach($oOrderInfo->items as $oItem) {
            ...
            $sTmpTax = round(($oItem->tax_rate * 100) / 
                             ($oItem->price * round($oItem->qty, 2)), 2) * 100;
     ...
     * @return string 
     */
    function dibsflex_api_hextostr($hex) {
       $string = "";
    ...
    /**
     * Fixes cookie to escape MAC comparation errors
    ...
    function dibsflex_api_callbackPBB(&$aFields) {
        ...
        if (isset($_POST['customerId'])) {
            $aFields['PBB_customerId'] = $_POST['customerId'];
        }
        else {
            $aFields['PBB_customerId'] = '-';
        }

        if (isset($_POST['deliveryAddress'])) {
            $aFields['PBB_deliveryAddress'] = iconv("ISO-8859-1","UTF-8",$_POST['deliveryAddress']);
        }
        else {
            $aFields['PBB_deliveryAddress'] = '-';
        }
			
        if (isset($_POST['deliveryCountryCode'])) {
            $aFields['PBB_deliveryCountryCode'] = iconv("ISO-8859-1","UTF-8",$_POST['deliveryCountryCode']);
        }
        else {
            $aFields['PBB_deliveryCountryCode'] = '-';
        }

        if (isset($_POST['deliveryPostalCode'])) {
            $aFields['PBB_deliveryPostalCode'] = iconv("ISO-8859-1","UTF-8",$_POST['deliveryPostalCode']);
        }
        else {
            $aFields['PBB_deliveryPostalCode'] = '-';
        }
            
        if (isset($_POST['deliveryPostalPlace'])) {
            $aFields['PBB_deliveryPostalPlace'] = iconv("ISO-8859-1","UTF-8",$_POST['deliveryPostalPlace']);
        }
        else {
            $aFields['PBB_deliveryPostalPlace'] = '-';
        }
            
        if (isset($_POST['firstName'])) {
            $aFields['PBB_firstName'] = iconv("ISO-8859-1","UTF-8",$_POST['firstName']);
        }
        else {
            $aFields['PBB_firstName'] = '-';
        }
            
        if (isset($_POST['lastName'])) {
            $aFields['PBB_lastName'] = iconv("ISO-8859-1","UTF-8",$_POST['lastName']);
        }
        else {
            $aFields['PBB_lastName'] = '-';
        }
    }
    ...
    function dibsflex_api_errCodeToMessage($iErrCode) {
        ...
        switch ($iErrCode) {
            case 11:
                $sErrMessage = $this->dibsflex_helper_getlang('text_err_11');
            break;
            case 12:
                $sErrMessage = $this->dibsflex_helper_getlang('text_err_12');
            break;
            case 21:
                $sErrMessage = $this->dibsflex_helper_getlang('text_err_21');
            break;
            case 22:
                $sErrMessage = $this->dibsflex_helper_getlang('text_err_22');
            break;
            case 31:
                $sErrMessage = $this->dibsflex_helper_getlang('text_err_31');
            break;
            case 32:
                $sErrMessage = $this->dibsflex_helper_getlang('text_err_32');
            break;
            case 41:
            case 42:
                $sErrMessage = $this->dibsflex_helper_getlang('text_err_41');
            break;
            default:
    ...
    function dibsflex_api_callback($oOrder) {
        ...
        $oOrderInfo = $this->dibsflex_helper_getOrderObj($oOrder);
        
        $mStatus = $this->dibsflex_helper_dbquery_read("SELECT `status` FROM `" . 
                                             $this->dibsflex_helper_getdbprefix() . 
                                            "dibs_orderdata` WHERE `orderid` = '" . 
                                             $oOrderInfo->order_id . "' LIMIT 1;");
    ...
    /** START OF CGI API **/
        ...
        function dibsflex_api_cgibuttons($sOrderId) {
            ...
            if($sTransac != "") $sState = $this->dibsflex_api_payinfo($sTransac);
            ...
            if (strpos($sState, "&") !== FALSE && strpos($sState, "=") !== FALSE) {
                ...
                if($oState->status == 2 || $oState->status == 5) {
                    $sOutput .= '<tr><td style="vertical-align: top; padding: 5px 5px 5px 0px;">
                                ...
                                <form id="dibsflex_cgiform" action="' .  $this->dibsflex_helper_getReturnURLs('cgi') . 
                                '" method="POST">
    ...
    function dibsflex_api_cgiapi() {
        ...
        if(isset($_POST['cgicancel'])) {
            $sAPI = 'cancel';
            $sApiLink = 'https://' . $sApiLogin . ':' . $sApiPass . '@payment.architrade.com/cgi-adm/cancel.cgi';
        }
        ...
        elseif(isset($_POST['cgirefund'])) {
            $sAPI = 'refund';
            $sApiLink = 'https://' . $sApiLogin . ':' . $sApiPass . '@payment.architrade.com/cgi-adm/refund.cgi';
        }
    ...
    function dibsflex_api_payinfo($sTransac) {
        $sApiLogin = $this->dibsflex_helper_getconfig("apiuser");
        $sApiPass = $this->dibsflex_helper_getconfig("apipass");
        $sResult = $this->dibsflex_api_postcgi("https://".$sApiLogin.":".$sApiPass."@payment.architrade.com/cgi-adm/payinfo.cgi", 
