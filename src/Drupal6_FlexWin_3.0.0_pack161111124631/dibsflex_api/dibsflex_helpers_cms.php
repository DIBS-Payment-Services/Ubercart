...
function uc_dibsflex_getTextArray() {
    $aText = array(
        'text_err_fatal'     => t('A fatal error has occured!'), 
        'text_return_toshop' => t('Return to shop'), 
        'text_err_11'        => t('Unknown orderid was returned from DIBS payment gateway!'), 
        'text_err_12'        => t('No orderid was returned from DIBS payment gateway!'), 
        'text_err_21'        => t('The amount received from DIBS payment gateway 
                                 differs from original order amount!'), 
        'text_err_22'        => t('No amount was returned from DIBS payment gateway!'), 
        'text_err_31'        => t('The currency type received from DIBS payment gateway 
                                 differs from original order currency type!'),
        'text_err_32'        => t('No currency type was returned from DIBS payment 
                                 gateway!'), 
        'text_err_41'        => t('The fingerprint key does not match!'), 
        'text_err_def'       => t('Unknown error appeared. Please contact to shop 
 ...
 * @return int 
 */
function uc_dibsflex_getitemtax($aTaxes) {
    $iSumm = 0;
    foreach($aTaxes as $oTax) {
        $iSumm += round($oTax->taxed,2) * 100;
    }
    return $iSumm;
}
 ...
 * @return int 
 */
function uc_dibsflex_shippingtax($oOrder) {
    $iSumm = 0;
    $aLines = $oOrder->line_items;
    foreach($aLines as $aLine) {
        if($aLine['type'] == 'shipping' && $aLine['amount'] == $oOrder->quote['rate']) {
            if(isset($aLine['tax'])) $iSumm = round($aLine['tax']->taxed, 2) * 100;
            else $iSumm = 0;
        }
    }
    return $iSumm;
...
function uc_dibsflex_action_apply_tax($order, $tax) {
    $amount = 0;
    $taxable_amount = 0;
    ...
    if (is_array($order->products)) {
        foreach ($order->products as $key => $item) {
            ...
            $oTmpData = (object)array(
                'name' => $tax->name,
                'taxed' => $iAmountBuffer * $tax->rate
            );
            $order->products[$key]->tax[] = $oTmpData;
    ...
    if (is_array($order->line_items) && is_array($taxed_line_items)) {
        foreach ($order->line_items as $key => $line_item) {
            ...
            if (in_array($line_item['type'], $taxed_line_items)) {
                $oTmpData = (object)array(
                    'name' => $tax->name,
                    'taxed' => $line_item['amount'] * $tax->rate
                );
